...
**RULE**: Clips total MUST be >= audio total + 2 seconds (end buffer so narration doesn't run into black screen). If gap exists:
1. Add an additional clip from DB (must not already be in the plan)
2. Extend last clip's duration (if only slightly short)
3. Slow down 1-2 clips (0.8x)

**NEVER proceed to assembly with clips shorter than audio + 2s.**
...
- [ ] Clips total duration >= audio duration + 2s buffer (confirmed via `$GIVORE_TOOLS clips plan`)
